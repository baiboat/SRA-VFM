...
###################### Data and Model ########################
from models.SAM_CD1 import SAM_CD as Net
NET_NAME = 'SAM_CD'
...
def main():
    ...
    #checkpoint_path = '/home/zwc/Downloads/SAM-CD-main/checkpoints/pretrain/SAM_CD_e6_OA96.04_F54.92_IoU44.81.pth'   # 替换为您保存的权重文件的路径
    #checkpoint = torch.load(checkpoint_path, map_location=torch.device('cuda'))
    #net.load_state_dict(checkpoint, strict=False)
    train_set = RS.RS('train', random_crop=True, crop_nums=20, crop_size=args['crop_size'], random_flip=True, random_regroup=True, rand_exchange_CD=True) #'5_train_supervised', 
...
def train(train_loader, net, optimizer, val_loader):
    ...
    while True:
        ...
        for i, data in enumerate(train_loader):
            ...
            imgs_A, imgs_B, labels, edges = data
            if args['gpu']:
                ...
                edges = edges.to(torch.device('cuda', int(args['dev_id']))).float().unsqueeze(1)

            optimizer.zero_grad()
            outputs, outedges, outA, outB = net(imgs_A, imgs_B)

            assert outputs.shape[1] == 1
            loss_bn0 = F.binary_cross_entropy_with_logits(outputs, labels)
            loss_bn1 = F.binary_cross_entropy_with_logits(outedges, edges)
            loss_t = criterion_sem(outA, outB, labels)
            loss = loss_bn0 + loss_bn1*2 + loss_t
            loss.backward()
            optimizer.step()
...
def validate(val_loader, net, curr_epoch):
    ...
    for vi, data in enumerate(val_loader):
        ...
        with torch.no_grad():
            outputs, edge, outA, outB = net(imgs_A, imgs_B)
